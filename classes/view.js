...
var view = function(world){
    ...
    this.rescaleSize.oninput = function(e){
        ...
        world.positiveLimit = {x: world.size.x + world.scene.look.x, y: world.size.y + world.scene.look.y, z: world.size.z + world.scene.look.z};
        world.negativeLimit = {x: -world.size.x + world.scene.look.x, y: -world.size.y + world.scene.look.y, z: -world.size.z + world.scene.look.z};
    ...
    this.resize = function(){
        ...
        self.loadingMessage.style.marginTop = (self.canvas.height / 2) - 20;
        ...
        if(self.isWorldMenuFix || self.isSettingsFix || self.isAddMenuFix || self.isHomeMenuFix){
            ...
            if(self.isHomeMenuFix){
                self.homeMenu.style.marginLeft = self.canvas.width-304;
                style = window.getComputedStyle(self.homeMenu);
            }else if(self.isWorldMenuFix){
                self.worldMenu.style.marginLeft = self.canvas.width-304;
                style = window.getComputedStyle(self.worldMenu);
            }else if(self.isSettingsFix){
                self.settingsMenu.style.marginLeft = self.canvas.width-304;
                style = window.getComputedStyle(self.settingsMenu);
            }else if(self.isAddMenuFix){
                self.addMenu.style.marginLeft = self.canvas.width-304;
                style = window.getComputedStyle(self.addMenu);
            }

            self.menu.style.marginLeft = parseInt(style.width) + 4;
            self.canvas.width -= parseInt(style.width) + 4;
            self.analysisMenu.style.width = self.canvas.width;
        }
