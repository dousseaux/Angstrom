<?php
    ...
    if($_GET["createpsf"] == "true"){
        $len = strlen($_GET["pdb"]) - 1;
        if($len > 5){
            $handle = $_GET["pdb"];
            $_GET["pdb"] = $handle[$len - 3] . $handle[$len - 2] . $handle[$len - 1] . $handle[$len];
        }

        shell_exec("wget -O buildPSF/temp.pdb " . "http://files.rcsb.org/download/" . $_GET["pdb"] . ".pdb");
        shell_exec("./buildPSF/doPsf.sh " . "temp.pdb");

        $out = shell_exec("find -name temp_autopsf.pdb");

        if($out){
            $_GET["pdb"] = "buildPSF/temp_autopsf.pdb";
            $_GET["psf"] = "buildPSF/temp_autopsf.psf";
        }else{
            $_GET["pdb"] = "buildPSF/temp_autopsf_tmpfile.pdb";
            $_GET["psf"] = "buildPSF/temp_autopsf_tmpfile.psf";
        }
    }
?>
...
<script id='pdb' type='text/pdb'><?php readfile($_GET["pdb"])?></script>
<script id='psf' type='text/psf'><?php readfile($_GET["psf"])?></script>
<script id='prm0' type='text/psf'><?php readfile($_GET["prm0"])?></script>
<script id='prm1' type='text/psf'><?php readfile($_GET["prm1"])?></script>
<script id='prm2' type='text/psf'><?php readfile($_GET["prm2"])?></script>
...
<script>
    ...
    window.onload = function(){
        ...
        w.constants.boostFactor = parseInt("<?php echo $_GET['boostFactor'] ?>");
        w.constants.timeStep = parseInt("<?php echo $_GET['timeStep'] ?>");
        w.maxSteps = parseInt("<?php echo $_GET['maxSteps'] ?>");
        w.temperature0 = parseFloat("<?php echo $_GET['temperature'] ?>");
        w.exec_thermostat = parseInt("<?php echo $_GET['thermostat'] ?>");
        w.energyTempFrequency = parseFloat("<?php echo $_GET['energyFrequency'] ?>");
        w.thermostatFrequency = parseFloat("<?php echo $_GET['thermostatFrequency'] ?>");
        w.renderFrequency = parseFloat("<?php echo $_GET['renderFrequency'] ?>")*w.constants.boostFactor;
        w.view.mouse.mode = "<?php echo $_GET['mouseMode'] ?>";
        w.drawingMode = "<?php echo $_GET['drawingMode'] ?>";
        w.colorMode = "<?php echo $_GET['colorMode'] ?>" ;
        w.atomResolution = parseInt("<?php echo $_GET['atomResolution'] ?>");
        w.bondResolution = parseInt("<?php echo $_GET['bondResolution'] ?>");
        w.size0 = "<?php echo $_GET['worldSize'] ?>";
        w.exclude13 = parseInt("<?php echo $_GET['exclude13'] ?>");
        w.constants.epsolonR = parseFloat("<?php echo $_GET['epsolonR'] ?>");
        if("<?php echo $_GET['enableAdd'] ?>" == "true") w.view.isAddEnabled = true;
        if("<?php echo $_GET['martine'] ?>" == "true") w.isMartine = true;
        if("<?php echo $_GET['visualize'] ?>" == "true") w.vizualizationMode = true;

        /* Modify a little bit of the style for a better desktop view */
        w.view.container.style.margin = 0;
        w.view.container.style.marginRight = 4;
        w.view.container.style.borderLeft = "none";
        w.view.container.style.borderTop = "none";
        w.view.container.style.borderBottom = "none";
        w.view.status.style.marginBottom = 5;
...
<?php
    ...
    if($_GET["createpsf"] == "true"){
        ...
        shell_exec("rm " . $_GET["pdb"]);
        shell_exec("rm " . $_GET["psf"]);
        $_GET["createpsf"] == "false";
    }
?>
