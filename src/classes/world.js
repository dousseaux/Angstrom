...
World.prototype = {
    ...
    positiveLimit: {
        ...
        y: 0,
        z: 0
    }, // Positive limits of the world
    negativeLimit: {
        x: 0,
        y: 0,
        z: 0
    }, // Negative Limts of the world
    texsize: {
        x: 1,
        y: 1
    }, // Height and Width of the textures
    ...
    // ######### MODES
    colorMode: "atoms",
    /* Color rule used to color the particles. Modes:
    ...
    drawingMode: "bas",
    /* Mode that the particles and bonds will be drawn. Modes:
    ...
    sampleMode: "energy",
    /* Mode that sets the way that data will be sampled and output in a file. Modes:
    ...
    setup: function() {
        ...
        var prm_content = [];
        if (document.getElementById("prm0").text) prm_content.push(document.getElementById("prm0").text);
    ...
    addMols: function(particleSystem, pos) {
        ...
        for (var i = 0; i < particleSystem.nmolecules; i++) {
            this.data.molecules_Offset.push({
                firstp: particleSystem.moleculesOffset[i].firstp + this.natoms,
    ...
    select: function(selection, par, n) {
        if (selection === "clear") {
            ...
            for (var i = 0; i < this.natoms; i++) {
                for (var j = 0; j < par.length; j++)
                    if (this.data.atoms_residuals[i] === par[j]) atoms.push(i);
            }
            if (this.drawAtoms) this.atoms.addIDs(atoms);
            if (this.drawBonds) {
                var bonds = [];
    ...
    recenter: function() {
        this.scene.camera = {
            x: this.scene.look.x,
            y: this.scene.look.y,
            z: this.scene.look.z + this.size.x + this.size.y + this.size.z * 2
        };
        this.scene.camera_rot = {
            x: this.scene.camera.x,
            y: this.scene.camera.y,
            z: this.scene.camera.z
        };
        this.scene.camera_rot = {
            x: this.scene.camera.x,
            y: this.scene.camera.y,
    ...
    center: function() {
        ...
        if (this.size0 === "auto") this.size = {
            ...
            this.size.y = arr[1];
            this.size.z = arr[2];
            this.scene.camera.z = zmean + xmax + ymax + 2 * zmax;
        }
        ...
        this.positiveLimit = {
            x: this.size.x + this.scene.look.x,
            y: this.size.y + this.scene.look.y,
            z: this.size.z + this.scene.look.z
        };
        this.negativeLimit = {
            x: -this.size.x + this.scene.look.x,
            y: -this.size.y + this.scene.look.y,
